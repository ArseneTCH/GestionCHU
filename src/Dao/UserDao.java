...
public class UserDao implements IDao<User>{
    ...
    private final String  SQL_SELCT_BY_Login = "SELECT * FROM user WHERE login =  ?";
    ...
    public User findUserLoginAndPassword(String login,String password){
     ...
     public User findUserLogin(String login){
        User user = null;
        database.openConnexion();
        database.initPrepareStatement(SQL_SELCT_BY_Login);
        try {
            database.getPs().setString(1, login);
            ResultSet rs = database.executeSelect(SQL_SELCT_BY_Login);
